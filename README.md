...
OVERVIEW
--------
- SmallWorld is a text-driven adventure game developed as a class project to demonstrate Object design, reverse engineering, learning debugger, eclipse, GitHub, team communication skills, project management skills and more. The game is inspired by classic adventure games where the player types commands to explore a labyrinth. The main goal of SmallWorld is to navigate through a series of connected rooms, collecting and using items, solving riddles, and lastly making it to the final room to win the game. Each room has a specific requirement to enter or progress, such as needing a key to open a locked door or bringing a specific item to solve the riddle to pass the locked door. 

-The core features of the game include directional movement commands (such as: North, South, East West), item manipulation (such as: Pick up, drop, use), and dynamic riddles that require specific inventory items to solve. Players must pay attention to room descriptions and clues provided throughout the game to understand the next step to progress in the game. The game tracks inventory, current location, and riddle states using internal variables and data structures, forming a simple effective game loop. 

-From an architectural perspective, SmallWorld is built with modular code that separates controller, model, persistence, and textui. Controller deals with the functionality of  executing the user's commands. Model  deals with the direction, world observer, item, place, and player functionality of the code. Persistence holds the world XML files (Map of the world). textui deals with the visual aspect, the ui the user sees and types commands into. This structure makes it easier to isolate and test specific components of the game independently.  

-The game is developed in Java using Eclipse, and testing will ensure that gameplay remains smooth, logical, and error free as players move through the maze-like environment.
